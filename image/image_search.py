...
@app.route('/feature_search', methods=['POST'])
def feature_search():
    ...
    print([data[0]['vec']])
    result = collection.search([data[0]['vec']], "feature", search_params, limit=3, output_fields=["random"])
    end_time = time.time()
    ...
    for hits in result:
        for hit in hits:
            ...
            results.append({"id": hit.id, "distance": hit.distance})
    print(results)
